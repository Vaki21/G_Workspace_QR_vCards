    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (users != null && users.Count > 0)
            {
                ...
                var privateKey = new PrivateKeyFile(@"PATH TO id_rsa FILE");

                //Path to the local directory and the directory I upload to
                const string workingDirectory = "";
                string newDirectory = workingDirectory + "/" + thisYear + "/" + thisMonth;
                var localDirectory = Directory.GetFiles("C:\\Users\\YOUR_USERNAME\\source\\repos\\QRUsers - Git\\QRUsers - Git\\bin\\Debug\\net6.0\\QRImages");
                ...
                using (var client = new SftpClient(host, username, new[] { privateKey }))
                {
                    ...
                    foreach (var files in listDirectory)
                    {
                        ...
                        //client.DeleteFile("workingDirectory" + kok);
                    }
